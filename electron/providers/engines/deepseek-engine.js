// Proxima — DeepSeek (chat.deepseek.com) Engine.
...
// effort selection (DeepSeek-V3 vs R1 / Deep Think) is NOT handled here
// (deferred to commit #2).
//
// Exposes window.__proximaDeepseek = { send, newConversation }.
...
(function () {
    ...
    var _sessions = {};
    try {
        var saved = localStorage.getItem('proxima_deepseek_sessions');
    ...
    function saveSession(sessionId) {
        ...
        try { localStorage.setItem('proxima_deepseek_sessions', JSON.stringify(_sessions)); } catch (e) { }
    }
    ...
    async function send(message, engine, effort, conversationId) {
        ...
        if (!answer || answer.length === 0) throw new Error('DeepSeek returned empty response');
        return answer;
    }
    ...
    function newConversation(sessionId) {
        ...
        try { localStorage.setItem('proxima_deepseek_sessions', JSON.stringify(_sessions)); } catch (e) { }
        console.log('[Proxima] DeepSeek conversation reset:', sessionId || 'current');
    }
